        ...
        public void Parse(Parser parser, List<Token> tokens, List<IParserState> states)
        {
            ...
            foreach (Token token in tail.ToList())
            {
                if (token.Type == TokenType.Identifier)
                {
                    ...
                    {
                        ParserUtils.CreateError(parser, token.StartPos, "Пропущено std::complex<double>");
                    }
                    break;
                }
            ...
            if (tail.Count > 0)
            {
                ParserUtils.CreateErrorFromBuffer(parser, errorBuffer, "Ожидалось std::complex<double>");
